...
import { useRdxWatcherLoader } from "../hooks/stateHooks";

/**
 *
 * 构造出一个Watcher，默认为loading状态，通过setValue通知节点加载完成，通过setLoading可以将节点设置
 * 为loading状态
 * @export
 * @param {{ id: string }} config
 * @returns
 */
export function pendingWatcher(config: { id: string }) {
  const { id} = config
