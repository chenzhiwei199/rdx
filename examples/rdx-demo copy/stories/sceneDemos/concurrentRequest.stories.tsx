...
import 'examples/portal-layout-demo/stories/sceneDemos/node_modules/antd/dist/antd.css';
export default {
  title: '场景专题/最大请求队列',
  parameters: {
    info: { inline: true },
...
export const 并发请求示例 = () => {
  ...
  const request = useMemo(() => buildMaxConcurrentReuqest(concurrentCount), [
    concurrentCount,
  ]);
  return (
    <div>
        ...
        onClick={() => {
          for (let index = 0; index < 10; index++) {
            setRequestList((requestList) =>
              requestList.concat({ id: index, finish: false })
            );
            request().then((res) => {
      ...
      {requestList.map((item) => {
        return (
          <div>
            请求id为{item.id}, 当前状态: {item.finish ? '请求完成' : '请求中'}
          </div>
        );
      })}
    </div>
