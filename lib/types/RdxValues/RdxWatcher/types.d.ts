import { IRdxNode, RdxGet, RdxReset, RdxSet } from '../base';
import { DataModel } from '../types';
/**
 * @export
 * @interface IRdxWatcherOperate
 * @template IModel
 */
export interface IRdxWatcherOperate<GModel> {
    get: IRdxWatcherGet<GModel>;
    set?: IRdxWatcherSet<GModel>;
}
export declare type IRdxWatcherNode<GModel> = IRdxNode & IRdxWatcherOperate<GModel> & {
    defaultValue?: GModel;
    virtual?: boolean;
};
export declare type IRdxWatcherGet<GModel> = (config: {
    id: string;
    /**
     * 当事件冲突时触发时候的回调
     *
     * @memberof ReactionContext
     */
    callbackMapWhenConflict: (callback: () => void) => void;
    value: GModel;
    get: RdxGet;
}) => DataModel<GModel>;
export declare type IRdxWatcherSet<GModel> = (config: {
    id: string;
    value: GModel;
    get: RdxGet;
    set: RdxSet;
    reset: RdxReset;
}, newValue: GModel) => void;
